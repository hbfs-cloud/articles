...
Each part stands on its own. This is 4 of 6 in The ETF Toolkit; earlier parts cover the groundwork but you can start here.
::end

Two rules, and people keep collapsing them into one. The contribution rule says how money arrives. The allocation rule says where it sits. <mark>Automating the first does nothing for the second.</mark>
...
![Drift is corrected by where new money goes, not by selling](https://raw.githubusercontent.com/hbfs-cloud/articles/main/substack-assets/examples/etf-toolkit_episode-04.png)
...
1. Set the contribution amount, the date and the eligible funds.
1. Record target weights and the rebalancing band right next to them.
1. Send new cash to the underweight sleeve first.
1. Estimate spread, tax and commission before any sale.
1. Keep tactical positions on a separate line from the core allocation.
1. Log every exception instead of rewriting the rule after a loss.
