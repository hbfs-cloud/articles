...
The famous case still teaches the lesson best. In January 2021, GameStop's reported short interest ran near 140% of its float, and every squeeze story since has been written in its shadow. Notice what that figure actually was: a snapshot, published with a lag, of positions held on a settlement date already in the past.
...
- Label it: position, transaction, fail balance or borrow quote.
- Keep settlement, trade and publication dates side by side.
- Record market coverage and the denominator definition.
- Keep vendor estimates apart from FINRA and SEC fields.
- Reject any squeeze claim resting on one stale snapshot.
