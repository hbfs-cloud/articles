...
Size from that arithmetic, not from the average. Beta is a fitted average sensitivity; it is not a worst case. And a high-beta position can gap straight past your exit price, stop tracking its reference, and dry up in liquidity all in the same session, usually the one where you most wanted the relationship to hold.
...
- Define the reference shock, the leftover shock, a gap and slippage before ordering.
- Calculate the scenario loss in dollars, not percent, before the order goes in.
- Compare the scenario, the exit level, the concentration cap and what you can realistically sell.
- Cut or skip anything carrying an unmodelled binary event inside the horizon.
- Re-estimate only on fresh overlapping data, never on a window you chose after the fact.
- Record your actual fills and correct the assumptions that were wrong.
