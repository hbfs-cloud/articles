...
Each part stands on its own. This is 6 of 6 in Options Without the Hidden Risk; earlier parts cover the groundwork but you can start here.
::end

Traders rarely go broke on a bad strategy. <mark>They go broke on a good strategy sized wrong.</mark> Losing everything on a position is survivable at 2% of the account and fatal at 30%.
...
![The budget decides the contract count](https://raw.githubusercontent.com/hbfs-cloud/articles/main/substack-assets/examples/options-risk_episode-06.png)
...
1. State the maximum contractual loss and a separate gap or assignment stress.
1. Convert every quote into whole-position dollars with the contract multiplier.
1. Add commissions, bid-ask slippage and any share obligation into the total.
1. Group Greeks by underlying and expiry, then price named spot, time and volatility shocks in dollars; never add raw Greeks across different underlyings.
1. Fix the exit rule, the no-roll condition and the last acceptable day to act.
1. Confirm cash and margin after the stress, not before it.
...
> The budget is decided before the trade, and its job is to reject the ones that do not fit.
