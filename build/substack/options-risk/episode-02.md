...
- Record Delta, Gamma, Theta and Vega with the time the quote was taken.
- Turn each one into whole-position dollars, multiplier and contract count included.
- Shock the stock up and down, not just the way your thesis points.
- Test one day of decay, then a volatility rise and a volatility fall.
- Redo it near the strike and near expiry, where Gamma makes Delta move fastest.
...
The deeper limitation is that Greeks describe small steps around where things stand right now. A gap, a halt, a wide spread or a volatility repricing like Apple's 16-point drop leaves the estimate behind. Use the dials to expose what can hurt you, not to manufacture a forecast.
