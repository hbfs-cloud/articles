...
<mark>Promotion runs on mechanics, never on profit.</mark> Move up a tier only if fills came back whole at the expected price, slippage stayed inside the band you predicted, stops triggered correctly including on gap opens, and no circuit breaker fired. Three winning trades tell you the market was kind. They tell you nothing about the plumbing. A backtest showing a profit factor of 1.6 can land at 1.1 once real slippage and real fills are counted, and you want that discovery at 10% size.

Launch checklist:

1. Freeze the strategy and sizing versions with a timestamp.
1. Confirm per-trade, portfolio, concentration and leverage limits.
1. Test rejected orders, stale data and the emergency exit before you need them.
1. Reconcile every fill and fee against broker records, not against your chart.
1. Write down the observation window and the promotion gate for each tier.
1. Stage entries across four to eight weeks rather than one session.
1. Hold capital at the current tier while any exception stays open.
...
> Day one has no cushion — buy the information about slippage, fills and your own nerves at the smallest size you will ever pay for it.
