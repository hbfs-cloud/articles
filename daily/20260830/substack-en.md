# The Sunday Plan Before AI Spending Faces Its Next Test
...
The regime model remains favorable to risk with a score of 79% of its scale, not a 79% probability. The VIX closed at 14.43 and implied volatility rises with the measurement horizon; those index levels are not insurance premiums. That is a calm structure, not a panic structure.

The contradiction is participation. Large-cap indices remain close to their highs, while small caps weakened into Friday. Gold sold off sharply, long-duration Treasuries did not provide a clean final-session refuge, and the oil futures fund USO finished below its Monday close. This is selective risk appetite, not a broad all-clear.

Crypto does not resolve the contradiction. The weekend rebound is real, but the off-hours evidence remains inconclusive. Ether and Solana are rising while open interest falls, which is consistent with deleveraging or short covering rather than aggressive new leverage. Relative volume also remains light. A 24/7 market does not mechanically predict the US open.

The clean bullish confirmation would be Bitcoin and Ether strengthening together on better participation, followed by confirmation from the listed proxies. COIN currently has a 0.73 rolling correlation with Bitcoin and a 1.32 beta. MSTR has a 0.76 correlation and a 1.68 beta. HUT and IREN show insufficient correlation with Bitcoin in this capture. The alternative xyz:MSTR perpetual is not the official MSTR share price and must not be used as an opening quote.
...
No pre-earnings entry qualifies. The collected support and resistance data is not reliable enough to publish precise levels, and an overnight gap can jump over any stop.
...
5. A visible intraday base that defines the entry and invalidation.
...
The regime allows selective risk, but the calendar blocks pre-event execution. The correct status is **WAIT** until guidance, the gap, VWAP, volume, and the first post-event base can be observed.
