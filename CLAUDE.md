...
## MCP Social Media — `articles-social`

Serveur MCP Node.js (`/Users/marketwatchxyz/GolandProjects/articles-social-mcp/`) qui contrôle Chrome (profil persistant `~/.claude-chrome-profile`, profil nommé `claude`) via puppeteer-extra + stealth. Enregistré dans `.mcp.json`.

### Outils disponibles (préfixe `mcp__articles-social__`)

| Outil | Usage |
|-------|-------|
| `browser_open(url)` | Ouvre une URL dans Chrome |
| `browser_screenshot()` | Screenshot de la page actuelle |
| `browser_get_text()` | Texte visible de la page |
| `browser_click(selector)` | Clique sur un sélecteur CSS/XPath |
| `browser_type(selector, text)` | Saisit du texte dans un champ |
| `browser_evaluate(script)` | Exécute JS dans la page |
| `browser_wait(ms)` | Attend N millisecondes |
| `browser_new_tab(url?)` | Nouvel onglet |
| `browser_close()` | Ferme le navigateur |
| `social_post_twitter(text, url?)` | Rédige un tweet (max 280 chars) |
| `social_confirm_post_twitter()` | Confirme et publie le tweet |
| `social_post_linkedin(text, url?)` | Ouvre LinkedIn pour poster |
| `social_search_trends(query, platform)` | Recherche tendances (twitter/reddit/google) |
| `social_get_article_list(tab?)` | Liste articles récents du site |

### Commande : "Promouvoir l'article [URL/titre]"

1. Récupérer la liste via `social_get_article_list` si besoin
2. Rédiger les textes adaptés par plateforme :
   - **Twitter/X** : max 240 chars + URL (~23 chars), 1-2 hashtags (#marchés #trading #finance)
   - **LinkedIn** : 3-5 paragraphes, ton institutionnel, CTA final
   - **Reddit** : titre accrocheur, dans r/investing, r/FranceFinance, r/options selon le sujet
3. `social_post_twitter(text, url)` → screenshot → confirmation → `social_confirm_post_twitter()`
4. `social_post_linkedin(text, url)` → `browser_type` + `browser_click` pour compléter → screenshot → confirmation
5. **Toujours confirmer avec l'utilisateur avant publication définitive**

### Commande : "Idées d'articles" / "Tendances"

1. `social_search_trends(query, 'twitter')` sur les thèmes finance/trading actuels
2. `social_search_trends(query, 'reddit')` pour les discussions communautaires
3. `social_search_trends(query, 'google')` pour les actualités récentes
4. Proposer une liste d'idées avec titre, angle (beginner/expert), subreddit/hashtag source

### Règles importantes

- **TOUJOURS screenshot de vérification** avant publication
- **Ne jamais publier sans confirmation** de l'utilisateur
- Si Chrome n'est pas connecté → demander à l'utilisateur de s'authentifier manuellement dans la fenêtre Chrome ouverte
- URL articles : toujours `https://articles.market-watch.xyz/...`
- Langue : Français par défaut, anglais si la plateforme/subreddit est anglophone
