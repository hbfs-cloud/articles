...
Pour rendre une édition, préparer un `edition.json` avec sources, sections et jetons `[[claim:source:/pointeur:format]]`, puis utiliser `node tools/eur-evidence.js <dossier-external> <article-path>` pour rejouer les calculs et `node tools/render-eur-opportunities.js <edition.json>` pour générer HTML et `_data/claims.json`. Le graphique est recalculé depuis le classement mesuré, pas depuis une série éditoriale libre. Le gabarit est `tools/templates/eur-opportunities.html`. Vérifier les régressions avec `python3 -m unittest discover -s tools/tests -p 'test_eur*.py'`.
...
## Dividendes : contrôle obligatoire avant publication

Le classement swing par défaut mesure le rendement du cours après les coûts explicitement modélisés. Utiliser les OHLC corrigés des divisions de titres pour tous les rendements, SMA, ATR, extrema, gaps et niveaux. Les séries corrigées des dividendes ne doivent pas être mélangées à ces niveaux. Le modèle historique conserve les épisodes de détachement : les retirer sans preuve des dates d'annonce connues à l'entrée introduirait de l'anticipation.

Séparer trois objets : variation du prix ; créance de dividende acquise ; cash disponible après paiement, retenues, change et purification éventuelle. Pour une distribution ordinaire : `entrée < ex-date <= sortie` donne droit au coupon ; l'achat à l'ouverture de l'ex-date ne donne pas ce droit. Ne jamais ajouter le coupon à un rendement de cours déjà ajusté des distributions. Le ratio de cours ajustés du fournisseur n'est pas exactement le rendement cash d'une détention sans réinvestissement. Les montants historiques déjà corrigés des divisions ne se corrigent pas une seconde fois. Réconciliation montant/facteur manquante, devise incertaine ou ajustement inexpliqué => diagnostic cash `null`, jamais zéro ; le classement de prix peut rester calculable.

Pour tous les candidats classés, obtenir des sources primaires sur l'ensemble de la fenêtre de détention effective : ex-date, record date, paiement, montant, devise de distribution, action ordinaire vs ADS, statut approuvé/proposé/provisoire. Examiner toutes les tranches annuelles approuvées ou le calendrier publié ; une recherche sans résultat ne prouve pas l'absence de dividende. Une attribution gratuite, un dividende spécial, une option cash/actions et un réinvestissement nécessitent une lecture propre des modalités. Un paiement dans la fenêtre avec ex-date antérieure ne donne aucun coupon au nouvel acheteur.

Construire l'artefact avec `python3 tools/eur-dividend-gate.py --help`, puis valider sa correspondance à l'étude, aux dates, aux preuves et à leurs hashes. Les audits normalisés conservent les sources et inconnues. Un calendrier incomplet retire la ligne des priorités d'entrée, tout en permettant une mention explicite comme recherche. Un ex-date dans l'horizon exige de reconstruire le plan de prix et de vérifier le traitement des ordres chez le courtier ; il n'interdit pas automatiquement l'entreprise. Comparer ampleur, coût réellement perdu et finalité, pas déduire aveuglément tout le dividende brut comme une perte patrimoniale.

Si le profil fiscal n'est pas connu, ne pas présumer résidence, enveloppe, courtier, crédit d'impôt, conversion FX ou purification. Continuer le classement de prix avec ces inconnues explicites ; ne pas promettre un net encaissé. La retenue fiscale ne remplace pas la purification religieuse. Le rendement du cours seul n'établit pas le classement d'une stratégie de rendement total.

Contrôler le navigateur avec `node tools/check-eur-browser.cjs <dossier-article>` (Chrome configurable par `EUR_CHROME_PATH`), puis inspecter les captures. Inclure les dividendes dans `eur-evidence.js`, lier les chiffres affichés aux preuves, rejouer `test_eur*.py`, puis exécuter `tools/eur-publication-check.py` après les revues sur le même HTML. À chaque nouvelle édition, reconstruire la fenêtre depuis les calendriers des places concernées (pas seulement les jours de semaine) et revalider toutes les annonces. La publication précédente n'est pas une validation du calendrier actuel.
