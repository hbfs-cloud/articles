...
**Couche 4 (6ᵉ retour lecteur) : densité & non-récurrence.** Ne PAS republier une thèse déjà sortie récemment sans apport neuf (le 5ᵉ post recyclait le 4ᵉ = tell majeur) — approfondir/nouvelles données/contre-argument ou ne pas publier. Densité : si l'article se résume à ≤3 phrases d'info réelle noyées dans 380 mots, c'est de l'emballage (là où les LLM excellent) → couper. Pas de définitions réflexes (« a call option is… »). **La valeur de DailyTickers = le moteur (rotation/flux/niveaux/invalidations), pas la prose** → par défaut format **data-forward** (chart + table flux + niveaux suivis), prose minimale, ou humain écrit la prose.
