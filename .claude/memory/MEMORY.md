...
- [MCP EU coverage gap → RÉSOLU](project_mcp_eu_coverage_gap.md) — 2026-07-11 : backfill MCP livré le soir même. OHLCV EU OK (AIR.PA 395 barres) + RunScreener region=EU renvoie des candidats → scanner `eu_smallcap` DÉBLOQUÉ (re-jouer eu-pea-scanner.mjs). Résiduels : pas de country dans rows screener (filtre PEA par ticker profile.country + exclure UK/.L) + market_cap=0 par endroits.
...
- [Migration local→MCP rollout](project_migration_local_to_mcp.md) — POC #1 (mode factor) migré local→MCP via agent-staging --ingest (8703c31ec), pattern prouvé. Reste GATED : dtx-backed=ROI nul, parité Go (etf/trendline)=risque A/B strict en dernier, EU (etf_eu)=bloqué, crypto/casablanca=fetch-direct hors scope. Plan : docs/specs/migration-local-to-mcp.md. Pas de mass-delete.
