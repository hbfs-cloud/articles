...
- [dtx MCP](reference_dtx_mcp.md) — VOIE CANONIQUE (2026-07-08) : backtest/décision/régime systematic-tss via MCP `systematic.dailytickers.com` (DtxListConfigs/DtxReplay/DtxDecide/DtxRegime, cache OHLCV chaud=déterministe). Binaire local vendoré = fallback offline only. Consumer `dtx-engine.js` a un patch transport MCP prêt.
- [dtx MCP Wiring + Parité](project_dtx_mcp_wiring.md) — WIRING 2026-07-08 : `tools/dtx-mcp-ingest.js` (ingère DtxDecide+DtxReplay → staging byte-compatible, `dtx-scan.js` exporte le schéma) ; préférence MCP-canonique/binaire-fallback par mode (Step 4d détecte `engineMode:mcp` frais → conserve). **Parité MIXTE** : `etf_eu` match parfait (flippé MCP) ; `etf_us` decide match/replay diverge ; `forex` replay identique/decide diverge (recence) ; `us_highvol`+`stockbox_nasdaq` decide **OOM `signal:killed`** (univers actions 2,4–5,2k titres) → binaire obligatoire. Cause : dérive adjusted-close (cache serveur plus frais) + garde-fou RAM serveur. MCP ≠ drop-in aujourd'hui.
