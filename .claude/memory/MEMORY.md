...
- [Certification ≠ vérité](feedback_certification_is_not_truth.md) — un artefact certifié peut être FAUX : le flux datait le PPI au 14 (BLS : le 10) et ignorait le FOMC des 15-16, pourtant dans l'horizon de toutes les lignes. Registre `data/scheduled-events.json` + gate `validate-horizon-risk.js`. Corollaire : un R/R issu d'une formule ne se présente pas comme une découverte du marché.
- [Marché de prédiction : lire la dérive, pas le niveau](feedback_prediction_market_read_the_drift.md) — daily 07/09 : cote de hausse de taux à 48,5 % publiée comme risque croissant alors que `price_change_7d` valait −0,04 (le marché s'en éloignait depuis 7 jours). Corollaires : `yes_price` = point milieu (bid/ask 0,48/0,49), `volume` = cumulé depuis création (utiliser `volume_24h`), queues à `spread_bps` > 500 non chiffrables, `scan_truncated` à divulguer
- [Mesurer une reprise depuis le cours d'AVANT](feedback_measure_recovery_from_pre_event_baseline.md) — daily 07/09 : « l'or a effacé sa chute » mesuré depuis la clôture d'après-chute (408,89) au lieu du cours d'avant (422,60) ; l'or finissait à −3,75 % avec un plus bas POSTÉRIEUR à 396,75. Piège du dénominateur choisi, gardé dans les formules `ratio_pct` mais pas dans la prose : 128/128 claims valides
- [Dérive du contrat marketdata 06e115d7](project_marketdata_contract_drift_20260906.md) — 2026-09-06 : facettes current-only refusent `end_date` (échec fermé), facette `quote` défaillante, absence non convertie en succès vide (`unusual_options` → vague détachée), ETF sans facettes par nom. Plus un vrai bug corrigé : `awaitJob()` sondait `Jobs` sans `maxsize` → toute page > 70 Ko renvoyait une erreur au lieu d'une page
- [Le contango n'est pas un signal](feedback_contango_is_not_a_signal.md) — VIX9D sous VIX30D = état normal de la courbe ; le weekly 20260914 en a fait une thèse de une et inversait la lecture de sa propre source (`shape:"contango"`). Publier le RATIO et sa variation (0,82 → 0,91 = court terme qui se renchérit), jamais le niveau.
- [Nombres en toutes lettres = contournement du gate](feedback_spelled_numbers_evade_claims_gate.md) — « vingt heures trente » échappait à validate-content-claims ; gate durci (cardinaux français) + rendu `fr_time` lié à `economic_events.json`. Tests A12/A13.
- [Marchés de prédiction dans regime.json](feedback_prediction_markets_in_regime_facets.md) — `facets.prediction_markets` + `facets.regime.transition_5d` collectés à chaque édition, presque jamais utilisés ; le FOMC du 16/09 était price à 78,5 % de hausse pendant que la page écrivait « sans inférer le sens ».
...
- [Preview must not bypass gates](feedback_preview_must_not_bypass_gates.md) — un aperçu change l'étiquette, jamais l'éligibilité ; et un zéro dû à un seuil doit se nommer, sinon il ressemble à une séance calme (scores plats à 80 depuis le 08/09 = zéro entrée pendant deux semaines, invisible).
- [RefreshBars casse le chemin corrélation](feedback_refreshbars_breaks_correlation_path.md) — le ré-ingest par symbole répare `bars_daily` mais laisse le titre à ~4 observations dans `PortfolioRisk(correlation)` ; recalculer localement depuis les barres certifiées et le déclarer.
- [distance_200dma_pct est en réalité une EMA200](feedback_distance_200dma_is_actually_ema200.md) — le plafond de 50 % est écrit en moyenne 200 jours, le champ mesure une exponentielle ; VLO ressortait à 47,5 % au lieu de 56,4 % et franchissait la porte sans qu'elle puisse le voir.
