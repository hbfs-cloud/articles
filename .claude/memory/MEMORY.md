...
- [Macro date verify before publish](feedback_macro_date_verify.md) — Toute date macro (CPI/FOMC/PPI/PCE/NFP) dans un digest/daily DOIT être vérifiée (MCP economic_events + WebSearch/BLS) AVANT publication ; jamais asserter un jour non vérifié. Le feed MCP economic_events est INCOMPLET (pas de CPI/FOMC → is_near_economic_event aveugle) — ticket owner docs/specs/mcp-economic-calendar-request.md. Incident : « CPI lundi » alors que c'était mardi 14/07.
