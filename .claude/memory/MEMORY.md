...
- [dtx Live Track & Drift](feedback_dtx_live_track_drift.md) — Modes scriptés : série live APPEND-ONLY (data/dtx-live-track.json, un point réel/soir écrit par gen-status-page, jamais interpolé) + drift backtest↔live (segment go-live→J extrait du replay COMPLET, |d|<2pp OK / 2-5 WATCH / >5 ALERT, fail-closed sans point échantillonné après go-live). Incident 13-21/07 : 2 semaines de live sans historique ni drift — la courbe sweep stagnait au 15/07. Garde qa-check 25c (72h).
- [Frozen Stats Append-Only Advance](feedback_frozen_stats_append_only_advance.md) — Stats hero LLM figées au 26/06 un mois durant : le fix immutabilité 02/07 avait retiré le recalcul config-blind SANS câbler l'avancement append-only (priorEC). Immutable ≠ figé : préfixe scellé byte-identique + segment neuf depuis les pnl scellés, garde préfixe sinon fallback. Avancement 21/07 : turbo 112→100,6%, dynamic 91→71%, balanced 49,6→40,5% (DD -13,9%).
...
- [Analysis Senior-Review First](feedback_analysis_senior_review_first.md) — senior-review AVANT publication + checklist MCP complète (buyback, 13F, série FINRA, EDGAR type=S-3) ; R/R attesté au pire fill de la zone ; correction de niveaux notifiés = notification de mise à jour. Incident TLN 19/07/2026.
...
- [DTX Frozen-Orders Guard](feedback_dtx_frozen_orders_guard.md) — Garde anti-gel dtx-mcp-ingest.js (exit 8) : DtxDecide a renvoyé des CREATE figés à J-9 ré-ingérés en silence 09→21/07 (gardes de fraîcheur sur ENTRÉES, jamais sur la SORTIE moteur). Trip si date de calcul ≠ asof OU batch CREATE byte-identique à une séance différente → staging NON écrit, exit 8, précédent conservé stale. Pas de faux positif (1er run / re-run même asof).
- [Perf Parallel MCP](project_perf_parallel_mcp.md) — Doctrine perf transverse (skill `perf-parallel-mcp`) : le goulot des skills = round-trip MCP en série. Isoler le MCP en salves // (1 message = N tool_use), batch QueryData multi-symboles, scripter l'assemblage node (scan-plan.js/scan-ingest-all.js), backgrounder le sweep. /scanner ≤5 min + appelle signals-desk (handoff). Invariants HARD STOP/fail-closed/no-skip inchangés.
