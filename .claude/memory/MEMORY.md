...
- [MCP EU coverage gap](project_mcp_eu_coverage_gap.md) — Diag 2026-07-11 : MCP marketdata sert l'EU en per-ticker (QueryData profile.country/mcap/EUR) MAIS ~3 barres OHLCV EU (earliest 2026-07-08) + aucune énumération EU (RunScreener region=eu → 0, GetReferentialData region=eu → DB US). Scanner `eu_smallcap` BLOQUÉ (spec figée), NE PAS re-tenter avant backfill MCP.
