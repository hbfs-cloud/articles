...
5. **⛔ SANITY GATE sur les métriques replay (données aberrantes ≠ échec technique)** : après ingest, lire les métriques du mode. Un run peut « réussir » techniquement mais renvoyer des **données cassées**. Marquer le mode **SUSPECT** (et NE PAS le publier comme fiable) + **alerte Telegram** si l'une de ces bornes saute : `max_dd_pct > 50` (un DD > 50 % sur un mode ETF/forex = données MCP cassées, PAS un backtest normal — ex. incident etf_eu DD=89,62 %), OU `sharpe < 0`, OU `total_trades` incohérent (>2× ou <0,3× le compte typique historique du mode), OU `cagr_pct`/`win_rate` hors bornes plausibles. Comparer aux valeurs de référence (les runs précédents de `data/dtx/<id>.json`). Ne JAMAIS publier en silence un mode dont les métriques sont aberrantes — c'est exactement le genre de garbage que « le MCP fait foi » ne doit PAS laisser passer. (Idéalement : garde déterministe à ajouter dans `dtx-mcp-ingest.js` — écrire un champ `_sanityWarning` que `qa-check.js` escalade.)
