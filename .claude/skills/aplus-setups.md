...
- **NEVER floor liquidity in SHARES — floor it in DOLLARS.** A share-count floor is a price filter in disguise and it silently deletes the best names. War room 2026-08-05: a 1.2–2.5M-share floor hid FFIV, which trades ~714k shares but **~$294M/day**, passes all four éliminatoires and has no dilutive filing. Same for CASY (337k shares = $287M/day) and MUSA (263k = $155M/day). Use `close * avg_volume > 20e6`.
- **`macd>0` tests the LEVEL of the MACD line, not the crossover.** It does not select momentum. Verified 2026-08-05: BCS MACD 0.3011 *below* its signal 0.3907, CL 0.1180 below 0.2633 — both finalists were decelerating and both passed a filter believed to gate on momentum. Compare `macd` to `signal` per-ticker in code (both come back in `QueryData types=technicals`).
- **Cover the RSI band you actually claim.** The structure axis allows RSI 50–68; a screen capped at 60 cannot see a name at 62 sitting on a rising EMA20, which is the ideal A+ shape. Run complementary passes, then let the extension gate do the rejecting.
...
- `QueryData types=earnings_quarterly limit=8` → keep only **≥5 consecutive beats**. Also check: **did the company raise guidance?** (search earnings call summary or `QueryData types=news` for "raised guidance" / "raised outlook"). No guidance raise = cap at A.
  - ⚠️ **The feed serves only 4 quarters**, whatever `limit`/`days` you pass (verified on BCS, CL, FFIV, STX, CASY, XOM). So "≥5 consecutive" is NOT verifiable from data alone. Either source the 5th quarter explicitly and **say in the article that it comes from a press source, not the data**, or the run is applying a 4/4 rule. Do not print "5 consecutive beats — verified" off a journalist's sentence. Open item: raise the history or rewrite the threshold to 4.
  - ⚠️ **Cross-check any name that reported in the last ~15 days.** The feed can be wrong. 2026-08-05: it returned XOM 2Q2026 actual 4.11 vs est 3.629 (a beat); the 30 July release was $3.52 adjusted against ~3.68 consensus, i.e. a **miss**. Tell: the four served quarters summed to $8.86 while trailing PE 25.92 at $153.96 implies ~$5.92. **Sum the four quarters and reconcile against `trailingPE × price`** — a mismatch means the series is corrupt.
  - ⚠️ **Non-US issuers break the earnings-window check.** `sec_filings` 8-K item 2.02 does not exist for foreign private issuers (6-K/20-F: TNK, BUD, BCS) or the Canadian MJDS regime (40-F: CM, BMO). For any non-domestic name, date the release from the press release / company IR page, never from `sec_filings` alone. Same blind spot for the dilution check: an empty `sec_filings` result on a 6-K filer is **absence of data, not absence of dilution** — never write "no dilutive filings" off it.
  - The 10-session earnings window must be declared **inclusive or exclusive** before the run. RNR (8-K item 2.02 filed 22/07) lands exactly on the boundary for a 05/08 session.
