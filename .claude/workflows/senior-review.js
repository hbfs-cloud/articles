...
const P = {
  ...
  quant: { label:'Quant Senior (data integrity)', mandate:`Verify EVERY hard number against fresh ground truth and FIX wrong ones. ToolSearch "select:mcp__claude_ai_marketdata__QueryData,mcp__claude_ai_marketdata__CheckJobStatus" and re-fetch the relevant data (quote, technicals, earnings_quarterly, stats, financials, bars_daily) for any ticker/figure cited. Check: prices/levels, market cap, P/E, EV/EBITDA, PEG, beta, margins, EPS beats (actual vs estimate), index/sector/commodity moves, and all computed ratios (R/R, %, win rate, profit factor) recompute correctly. Flag and correct any fabricated, stale, or internally-inconsistent number. For performance reporting (retro/scanner), confirm headline stats reconcile with the underlying table — never let a phantom win/return stand.` },
