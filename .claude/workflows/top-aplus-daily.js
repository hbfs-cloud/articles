...
const MCP_HINT = `Pour les données marché, utilise les outils MCP DailyTickers (charge leur schéma via ToolSearch : query "select:mcp__claude_ai_DailyTickers__QueryData"). QueryData accepte des symboles séparés par virgule et types= : quote (prix live), technicals (EMA20/50/200, RSI14, ATR), stats (pegRatio, enterpriseToEbitda, fwd P/E, shares, shortPercentOfFloat, beta), earnings_quarterly (8 derniers, vérifier 4/4 beats), flags (compliance/halted), news. ZÉRO HALLUCINATION : si une donnée n'est pas obtenue, ne l'invente pas — marque data_ok=false. Tous les prix doivent être à la MÊME ÉCHELLE que le quote live (leçon KLAC : un split rend les niveaux publiés faux).`
