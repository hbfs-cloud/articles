...
for(const n of ['status','bars','fundamentals','comparison_bars','comparison_context','rank_beta','sec_evidence'])input(n,run+'/data/'+n+'.json');
...
function sourceFor(p,value){
 ...
 if(/^meta\.(lastMcpRefresh|levelsVerifiedAt)$/.test(p)||p==='blastRadius.observationTime')return provenance('status','/captured_at','End-of-collection timestamp from the hash-bound sibling harness; distinguish it from status capture and market close.');
 ...
 if(/^blastRadius\.groups\.\d+\.symbols\.\d+\./.test(p)){
  ...
  if(field==='eventRisk')return provenance('comparison_context',facet('comparison_context','stats',row.ticker),'Qualitative event-risk disclosure only; no provider date or options-implied probability is published.');
