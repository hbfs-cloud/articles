...
Duration puts a number on the sensitivity. The working rule: percentage price change equals minus duration multiplied by the yield change. Duration 7, yields up one point, expect roughly minus 7%. Desks scale that into dollars. A $10 million book with duration 7 loses about $7,000 per basis point of yield. That figure, the DV01, is how a rates position gets sized in practice.
...
- Verify whether the published figure is effective or modified duration.
- Pair duration with the exact yield and benchmark used.
- Check credit quality and embedded options.
- Test both a rise and a fall in yields.
- Add income, expenses and the intended holding period.
