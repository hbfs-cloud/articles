...
const paths=['analyses/AMZN/_data/bars.json','analyses/AMZN/_data/comparison_bars.json'];
...
const target=normalizeBars({...targetRaw,bars:targetRaw.bars.filter(r=>r[0]>=start&&r[0]<=ref)},'AMZN',EQUITY_CALENDAR);
...
const result={kind:'independent_comparison_audit',ticker:'AMZN',referenceClose:ref,inputs,formula:'OLS comparator log returns on AMZN log returns; beta=cov(y,x)/var(x), R2=Pearson correlation squared. Complete identical US sessions required. No filling missing rows.',reference:{return5d:performance(targetRaw,5),return21d:performance(targetRaw,21)},comparisons:comps.map(r=>({ticker:r.symbol,return5d:performance(r,5),return21d:performance(r,21),...regression(r)}))};
fs.writeFileSync('analyses/AMZN/_review/comparison-audit.json',JSON.stringify(result,null,2)+'\n');
