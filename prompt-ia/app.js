...
(function() {
  ...
  // ── RENDER SELECTS (settings panel) ──
  function renderSelect(selectId, options, defaultVal) {
    ...
    options.forEach(function(o) {
      ...
      if (o.selected || o.value === defaultVal) opt.selected = true;
      sel.appendChild(opt);
    });
  }
  ...
  function renderAllControls() {
    ...
    // Settings = selects
    renderSelect('assetSelect', chips.asset[CURRENT_LANG]);
    renderSelect('aiSelect', chips.ai);
    renderSelect('levelSelect', chips.level[CURRENT_LANG]);
    renderSelect('formatSelect', chips.format[CURRENT_LANG]);
    renderSelect('langSelect', chips.reportLang[CURRENT_LANG]);
    renderSelect('focusSelect', chips.focus[CURRENT_LANG]);
  }

  // ── GET VALUES ──
  function getSelected(id) {
    // Try select first, then chip group
  ...
  document.getElementById('libToggle').addEventListener('click', function() {
    ...
    this.innerHTML = isHidden
      ? '<i class="fa-solid fa-chevron-down"></i> ' + L.libClose
      : '<i class="fa-solid fa-chevron-down"></i> ' + L.libOpen;
  });
  ...
  // ── DEEP LINKS (copy prompt + open AI homepage) ──
  // URLs with ?q= cause 4xx for long prompts. Instead: copy to clipboard, then open homepage.
  var aiHomeUrls = {
  ...
  function openInAi(ai, prompt) {
    clipCopy(prompt, function() {
      ...
      setTimeout(function() { window.open(aiHomeUrls[ai], '_blank'); }, 300);
    });
  }
  ...
  function renderOpenAiButtons(primaryAi, prompt) {
    ...
    var others = Object.keys(aiHomeUrls).filter(function(k) { return k !== primaryAi; });
  ...
  document.getElementById('copyBtn').addEventListener('click', function() {
    ...
    var selectedAi = getSelected('aiSelect');
    var rec = getAiRecommendations(getSelected('assetSelect'), getSelected('intentGroup'));
  ...
  function renderLibrary() {
    ...
    LIBRARY.forEach(function(p) {
      ...
      var badge = p.badge[CURRENT_LANG] || p.badge.en;
      var desc = p.desc[CURRENT_LANG] || p.desc.en;
      var code = p.code[CURRENT_LANG] || p.code.en;
      var sourceLabel = p.source[CURRENT_LANG] || p.source.en;
      ...
      card.innerHTML =
        ...
        '</div>' +
        '<div class="prompt-lib-body">' +
          '<p class="prompt-lib-desc">' + desc + '</p>' +
          '<div class="prompt-lib-code" style="position:relative">' +
            '<button class="prompt-lib-copy" onclick="copyLib(this)">' + L.copy + '</button>' +
            code +
          '</div>' +
          '<div class="prompt-lib-source"><i class="fa-solid fa-book"></i> <a href="' + p.source.url + '">' + sourceLabel + '</a></div>' +
        '</div>';
