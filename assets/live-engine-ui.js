...
(function () {
  ...
  // Phase B unification: bind MODE_PANEL_TPL into the server-emitted mp-host
  // placeholder for both Live and Time Machine. Server-rendered duplicate sections
  // are marked .legacy-hidden so this is the single visible layout.
  function renderMpHost(modeId, modeData, mCfg) {
    if (!modeData) return;
    // The inline-script that defines window.getModeTpl runs on DOMContentLoaded
    // AFTER live-engine-ui boot — retry on a short delay if globals aren't ready yet.
    ...
    // Update scenario bar fills if positions exist
  ...
  // Expose for Time Machine to call on date change
    ...
    Promise.all([
      ...
      return fetch('/scanner/status/history/' + latest + '.json?v=' + _v)
        ...
        .then(function (snap) {
          ...
          window._latestSnap = snap;  // exposed for Time Machine "back to live"

          Object.keys(cfg.modes).forEach(function (modeId) {
            createCard(modeId);
            buildPositionRows(modeId, allPositions[modeId]);
            reorganizePanel(modeId);
            // Phase B unify — fill the server-emitted mp-host with the canonical template,
            // bound to today's snapshot. Same template Time Machine reuses for historical dates.
                ...
                return fetch('/scanner/status/history/' + newest + '.json?v=' + Date.now())
                  ...
                  .then(function (newSnap) {
                    ...
                    Object.keys(fresh).forEach(function (modeId) {
                      ...
                      reorganizePanel(modeId);
                      // Re-bind unified mp-host with the new snapshot data.
