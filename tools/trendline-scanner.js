...
// ─── VOIE MCP (--ingest) — voie optionnelle data-path MCP (modèle EXACT factor/momentum --ingest) ──
// Quand --ingest est fourni, le scanner NE FETCH RIEN (ni Yahoo, ni cache) : il PARSE un staging JSON
// écrit par l'AGENT (qui, LUI, a appelé mcp__marketdata__* — OAuth2, zéro token). Le chemin LOCAL
// (Yahoo + univers local) reste PRIMARY ; --ingest est une voie MCP optionnelle, additive.
...
async function main() {
  ...
  // ni univers local). Le chemin LOCAL ci-dessous reste PRIMARY (défaut, --ingest absent).
