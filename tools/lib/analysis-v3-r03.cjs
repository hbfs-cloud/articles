'use strict';
// Gabarit commun des fiches v3 du lot R03 (PANW, ZS, TENB, SNOW, DDOG), refonte du 24 septembre 2026.
// Dérivé du gabarit R02 (inchangé) : ajoute un scénario de valorisation sur les revenus (logiciels
// dont l'EBITDA GAAP est faible ou négatif) et des pairs datés sur un multiple choisi par ligne.
...
const TEMPLATE = 'tools/lib/analysis-v3-r03.cjs', XBRL_LIB = 'tools/lib/xbrl-ttm.cjs';
...
function build(cfg) {
  ...
  // Repli : parmi les cassures non achetables à l'ouverture (clôture ou ouverture suivante au-delà du seuil), part
  // de celles où le cours est revenu sous le seuil dans les `k` séances suivantes, et part où le stop (en % sous le
  // niveau) a été touché la même séance que ce retour.
  const retestStats = (pctBuf, pctStop, k = 3) => { let miss = 0, back = 0, stopSame = 0;
    for (let i = 21; i < N - k; i++) { let lvl = -Infinity; for (let j = i - 20; j < i; j++) lvl = Math.max(lvl, bars[j][2]);
      if (bars[i][4] > lvl && bars[i - 1][4] <= lvl) { const lim = lvl * (1 + pctBuf / 100);
        if (bars[i][4] > lim || bars[i + 1][1] > lim) { miss++; for (let q = i + 1; q <= i + k; q++) if (bars[q][3] <= lim) { back++; if (bars[q][3] <= lvl * (1 - pctStop / 100)) stopSame++; break; } } } }
    return { miss, back, stopSame, k }; };
  ...
  const ctx = { nb, gapStats, breakoutStats, retestStats, peerEv, peerStat, extra, T, REF, K, fr, usd, pct, sgn, dfr, close, prev, bars, N, tech, st, M, adv, ret, G$, g, shares, marketCap, ev, scn, lv, entry, stop, tp1, tp2, abandon, cap, capRr, rr1, rr2, riskAtr, range, sizeExample,
  ...
  const srcMap = a.fundamentals.rows.map(r => ({ _src: r._src, _also: r._also, _peers: r._peers, _peerField: r._peerField || 'enterpriseToEbitda' }));
  ...
  const aOut = JSON.parse(JSON.stringify(a)); aOut.fundamentals.rows.forEach(r => { delete r._src; delete r._also; delete r._peers; delete r._peerField; });
  const sourceForOut = p => { const fm = p.match(/^fundamentals\.rows\.(\d+)\./); if (fm) { const s = srcMap[+fm[1]]; return withPeers(s._src === 'gaap' ? gaap() : s._src === 'market' ? marketMath() : Pm(s._src, s._also), s._peers, s._peerField); } return sourceFor(p); };
