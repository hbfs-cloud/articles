'use strict';
// Gabarit commun des fiches v3 du lot R02 (AVGO, MU, ANET, AMD, SMCI), refonte du 24 septembre 2026.
...
const TEMPLATE = 'tools/lib/analysis-v3-r02.cjs', XBRL_LIB = 'tools/lib/xbrl-ttm.cjs';
...
function build(cfg) {
  ...
  const scn = { multiple: cfg.scenarioMultiple, ebitda: G$.ebitda, debt: G$.debt, cash: G$.cash, shares, close };
  ...
  const peerEv = sym => { const q = peerPtr[sym]; if (q == null) throw Error('pair absent ' + sym); return at(peerRaw, q).enterpriseToEbitda; };
  const extra = Object.fromEntries((cfg.extraInputs || []).map(x => [x.name, read(x.path)]));
  const nb = n => String(Math.round(n)).replace(/\B(?=(\d{3})+(?!\d))/g, '\u00a0');
  const ctx = { nb, gapStats, peerEv, extra, T, REF, K, fr, usd, pct, sgn, dfr, close, prev, bars, N, tech, st, M, adv, ret, G$, g, shares, marketCap, ev, scn, lv, entry, stop, tp1, tp2, abandon, cap, capRr, rr1, rr2, riskAtr, range, sizeExample,
  ...
  const srcMap = a.fundamentals.rows.map(r => ({ _src: r._src, _also: r._also, _peers: r._peers }));
  const withPeers = (base, peers) => peers && peers.length ? { ...base, method: base.method + ' Pairs : EV/EBITDA courant des statistiques du 24 septembre (non point-in-time).', additional_inputs: [...(base.additional_inputs || []), ...peers.map(sym => dep('context', peerPtr[sym] + '/enterpriseToEbitda'))] } : base;
  const aOut = JSON.parse(JSON.stringify(a)); aOut.fundamentals.rows.forEach(r => { delete r._src; delete r._also; delete r._peers; });
  const sourceForOut = p => { const fm = p.match(/^fundamentals\.rows\.(\d+)\./); if (fm) { const s = srcMap[+fm[1]]; return withPeers(s._src === 'gaap' ? gaap() : s._src === 'market' ? marketMath() : Pm(s._src, s._also), s._peers); } return sourceFor(p); };
  const res = K.writeEvidence({ ticker: T, ref: REF, outJson: OUT_JSON, outEvidence: OUT_EVIDENCE, calcPath: rev + '/numeric-evidence.json', generator: GEN, a: aOut, inputs, sourceFor: sourceForOut, score: cfg.score,
    valuation: { basis: cfg.valuationBasis, ...scn },
  ...
  console.log(`[${T}] claims=${res.claims} score=${scoreValue} close=${close} mode=${mode} entry=${entry} stop=${stop} tp1=${tp1} tp2=${tp2} rr1=${rr1.toFixed(2)} cap=${cap} riskATR=${riskAtr.toFixed(2)} ev/ebitda=${(ev / G$.ebitda).toFixed(1)} pe=${(marketCap / G$.ni).toFixed(1)} scn=${scn.price.toFixed(2)}`);
  return { a: aOut, ctx };
}
