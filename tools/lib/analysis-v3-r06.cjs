'use strict';
// Gabarit commun des fiches v3 du lot R05 (SNPS, INTC, AMKR, VICR, AAOI), refonte du 24 septembre 2026.
// Copie du gabarit R04 (inchangé, toujours utilisé par SNPS et INTC) avec un seul ajout : `cfg.xbrlInstance`.
// Quand les données XBRL agrégées de la SEC (companyfacts) n'ont pas encore intégré le dernier 10-Q, les
// faits de ce 10-Q sont lus dans son instance XBRL officielle (fichier `_htm.xml` du dépôt EDGAR, haché),
// convertis de façon déterministe au format companyfacts dans un fichier dérivé haché, et ajoutés en mémoire
// aux lignes de companyfacts avant le calcul des douze mois glissants. Seuls les contextes sans dimension
// (entité entière) et les unités en dollars sont retenus. Le fichier companyfacts n'est jamais modifié ; chaque
// agrégat garde le pointeur de la ligne réellement utilisée, dans companyfacts ou dans le fichier dérivé.
...
const TEMPLATE = 'tools/lib/analysis-v3-r06.cjs', XBRL_LIB = 'tools/lib/xbrl-ttm.cjs';
...
function build(cfg) {
  const T = cfg.ticker, REF = '2026-09-24', run = `analyses/${T}/_runs/20260925`, data = `analyses/${T}/_data`, rev = run + '/revision', prim = `analyses/${T}/_primary`;
  ...
  const XBRL = prim + '/companyfacts.json', cf0 = read(XBRL);
  let cf = cf0, inst = null;
  const supPtr = {};
  if (cfg.xbrlInstance) {
    const xi = cfg.xbrlInstance, rawPath = prim + '/' + xi.local, xml = bytes(rawPath).toString('utf8');
    const ctx = {};
    for (const m of xml.matchAll(/<context id="([^"]+)">([\s\S]*?)<\/context>/g)) {
      if (/<segment>/.test(m[2])) continue;
      const st = (m[2].match(/<startDate>([^<]+)<\/startDate>/) || [])[1], en = (m[2].match(/<endDate>([^<]+)<\/endDate>/) || [])[1], ins = (m[2].match(/<instant>([^<]+)<\/instant>/) || [])[1];
      ctx[m[1]] = ins ? { end: ins } : { start: st, end: en };
    }
    const facts = {};
    const usdUnits = new Set([...xml.matchAll(/<unit id="([^"]+)">\s*<measure>iso4217:USD<\/measure>\s*<\/unit>/g)].map(u => u[1]));
    for (const m of xml.matchAll(/<us-gaap:([A-Za-z0-9]+)\s([^>]*)>(-?[0-9.]+)<\/us-gaap:\1>/g)) {
      const cref = (m[2].match(/contextRef="([^"]+)"/) || [])[1], uref = (m[2].match(/unitRef="([^"]+)"/) || [])[1];
      if (!usdUnits.has(uref)) continue;
      const c = ctx[cref]; if (!c) continue;
      const rows = ((facts[m[1]] = facts[m[1]] || { units: { USD: [] } }).units.USD);
      const row = { ...(c.start ? { start: c.start } : {}), end: c.end, val: Number(m[3]), accn: xi.accn, form: xi.form, filed: xi.filed };
      if (!rows.some(r => r.start === row.start && r.end === row.end && r.val === row.val)) rows.push(row);
    }
    const derived = { kind: 'xbrl_instance_facts_v1', source_path: rawPath, source_sha256: sha(bytes(rawPath)), source_url: xi.url, accn: xi.accn, form: xi.form, filed: xi.filed,
      method: 'Contextes sans dimension, unités en dollars, valeurs telles que publiées dans l’instance XBRL ; doublons exacts retirés ; ordre du fichier source.', facts: { 'us-gaap': facts } };
    const derivedPath = prim + '/xbrl-instance-facts.json'; write(derivedPath, derived);
    inst = { path: rawPath, sha256: sha(bytes(rawPath)), url: xi.url, accn: xi.accn, derived_path: derivedPath, derived_sha256: sha(bytes(derivedPath)) };
    cf = JSON.parse(JSON.stringify(cf0));
    for (const [tag, f] of Object.entries(facts)) {
      const tgt = ((cf.facts['us-gaap'][tag] = cf.facts['us-gaap'][tag] || { units: {} }).units.USD = cf.facts['us-gaap'][tag].units.USD || []);
      if (tgt.some(r => r.accn === xi.accn)) continue;
      f.units.USD.forEach((r, j) => { supPtr[`/facts/us-gaap/${tag}/units/USD/${tgt.length}/val`] = `/facts/us-gaap/${tag}/units/USD/${j}/val`; tgt.push(r); });
    }
  }
  const g = {};
  ...
  const gaapFacts = Object.fromEntries(Object.entries(g).map(([k, v]) => [k, { tag: v.tag, value: v.value, parts: (v.parts || [{ value: v.value, end: v.end, form: v.form, accn: v.accn, pointer: v.pointer }]).map(q => supPtr[q.pointer] ? ({ value: q.value, start: q.start, end: q.end, form: q.form, accn: q.accn, xbrl_instance_pointer: supPtr[q.pointer] }) : ({ value: q.value, start: q.start, end: q.end, form: q.form, accn: q.accn, companyfacts_pointer: q.pointer })) }]));
  ...
  const primary = { kind: 'primary_sec_manifest_v1', ticker: T, as_of: '2026-09-25', inventory_count: cfg.inventory, inventory_screened_count: cfg.inventory,
    ...
    ...(inst ? { xbrl_instance: { ...inst, method: 'Faits du dernier 10-Q lus dans son instance XBRL officielle, companyfacts ne les contenant pas encore à la collecte ; seules les parts marquées xbrl_instance_pointer en proviennent.' } } : {}),
  ...
  const peerManifest = sym => { const d = fs.readdirSync(path.join(K.root, 'analyses', sym, '_runs')).filter(x => x === '20260925'); if (!d.length) throw Error('manifeste pair absent ' + sym); return 'analyses/' + sym + '/_runs/20260925/revision/primary-manifest.json'; };
  ...
  const ctx = { nb, peerFiche, gapStats, breakoutStats, retestStats, peerEv, peerStat, extra, T, REF, K, fr, usd, pct, sgn, dfr, close, prev, bars, N, tech, st, M, adv, ret, G$, g, shares, marketCap, ev, scn, lv, entry, stop, tp1, tp2, abandon, cap, capRr, rr1, rr2, riskAtr, range, sizeExample,
    ...
    market: name => ({ name: 'Données de marché datées (provenance hashée) : ' + name, url: K.evidenceUrl(T), date: '2026-09-25' }) };
