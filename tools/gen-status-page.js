...
document.addEventListener('DOMContentLoaded',function(){
  ...
  function tmBuildHTML(d,mCfg,modeId){
    var color=mCfg.color||'#94a3b8';
    var h='';
    h+='<div class="section-card" style="margin-bottom:1rem"><div class="ps">'
      +'<div class="ps-i"><div class="ps-v">'+(d.stats.ret>0?'+':'')+d.stats.ret.toFixed(2)+'%</div><div class="ps-l">Total Return</div></div>'
      +'<div class="ps-i"><div class="ps-v">'+d.stats.dd.toFixed(2)+'%</div><div class="ps-l">Max Drawdown</div></div>'
      +'<div class="ps-i"><div class="ps-v">'+d.stats.wr.toFixed(1)+'%</div><div class="ps-l">Win Rate</div></div>'
      +'<div class="ps-i"><div class="ps-v">'+d.stats.pf.toFixed(2)+'x</div><div class="ps-l">Profit Factor</div></div>'
      +'<div class="ps-i"><div class="ps-v">'+d.stats.trades+'</div><div class="ps-l">Closed Trades</div></div>'
      +'<div class="ps-i"><div class="ps-v">'+d.stats.avgHold.toFixed(1)+'d</div><div class="ps-l">Avg Hold</div></div>'
      +'</div></div>';
    h+='<div class="perf-hero" style="border-left:3px solid '+color+'">'
      +'<div class="perf-hero-left"><span class="perf-hero-label"><i class="fas fa-chart-line" style="color:'+color+';margin-right:.3rem"></i>Equity Curve</span></div>'
      +'<div id="tm-eq-'+modeId+'" style="width:100%;height:260px"></div></div>';
    ...
    if(d.closeNow&&d.closeNow.length>0){
      h+='<div class="cta-card cta-close"><div class="cta-header"><span class="cta-icon"><i class="fas fa-ban"></i></span>'
      ...
      d.closeNow.forEach(function(p){var pnl=p.pnlPct!==undefined?p.pnlPct:(p.return_pct||0);
        h+='<tr><td><b>'+p.ticker+'</b></td><td class="m">'+(p.scan_date?p.scan_date.slice(5):'\\u2014')+'</td><td class="hide-m">$'+(p.entry||0).toFixed(2)+'</td><td class="'+(pnl>=0?'pos':'neg')+'"><b>'+(pnl>0?'+':'')+pnl.toFixed(2)+'%</b></td><td class="am">'+(p.days_held||'?')+'d</td><td><span class="pill neg">CLOSE</span></td></tr>';
      });h+='</tbody></table></div>';
    }
    if(raiseSL.length>0){
      h+='<div class="cta-card" style="background:#f0f9ff;border:1.5px solid #bae6fd;border-left:4px solid #0284c7"><div class="cta-header"><span class="cta-icon" style="background:rgba(2,132,199,0.1)"><i class="fas fa-arrow-up-right-dots" style="color:#0284c7"></i></span>'
        +'<div><h3 style="color:#0284c7">Raise Stop Loss <span class="cta-badge" style="background:#0284c7">'+raiseSL.length+' targets</span></h3>'
        +'<p class="cta-sub" style="color:#0284c7dd">Break-even triggered</p></div></div>'
        +'<table class="t"><thead><tr><th>Ticker</th><th>Entry</th><th>P&L</th><th>Stop</th><th>Held</th></tr></thead><tbody>';
      raiseSL.forEach(function(p){var pnl=p.pnlPct!==undefined?p.pnlPct:(p.return_pct||0);
        h+='<tr><td><b>'+p.ticker+'</b></td><td>$'+(p.entry||0).toFixed(2)+'</td><td class="pos"><b>+'+pnl.toFixed(2)+'%</b></td><td><span class="pill pos" style="background:#0284c7;color:#fff">B.EVEN</span></td><td>Trailing</td></tr>';
      });h+='</tbody></table></div>';
    }
    if(d.expiresTomorrow&&d.expiresTomorrow.length>0){
      h+='<div class="cta-card" style="background:#fffbeb;border:2px solid #fcd34d;border-left:4px solid #f59e0b"><div class="cta-header"><span class="cta-icon" style="background:rgba(245,158,11,.15)"><i class="fas fa-hourglass-half" style="color:#d97706"></i></span><div>'
        ...
        +'<table class="t"><thead><tr><th>Ticker</th><th>Entry</th><th>P&L</th><th>Stop</th><th>Held</th></tr></thead><tbody>';
      d.expiresTomorrow.forEach(function(p){var pnl=p.pnlPct!==undefined?p.pnlPct:(p.return_pct||0);
        h+='<tr><td><b>'+p.ticker+'</b></td><td>$'+(p.entry||0).toFixed(2)+'</td><td class="'+(pnl>=0?'pos':'neg')+'"><b>'+(pnl>0?'+':'')+pnl.toFixed(2)+'%</b></td><td class="neg">'+(p.stop&&p.stop!==0?'$'+p.stop.toFixed(2):'EXIT')+'</td><td class="am">'+(p.days_held||'?')+'d</td></tr>';
      });h+='</tbody></table></div>';
    }
    if(d.orders&&d.orders.length>0){
      h+='<div class="section-card cta-orders"><div class="sc-head"><h3><i class="fas fa-bolt"></i> '+d.orders.length+' Order'+(d.orders.length===1?'':'s')+' to Place</h3></div>'
      ...
      d.orders.forEach(function(o){var bg=o.score>=90?'#059669':o.score>=85?'#2563eb':'#f59e0b';
        h+='<tr><td><b>'+o.ticker+'</b></td><td class="hide-m"><span class="pill-score" style="background:'+bg+'">'+o.score+'</span></td><td><b>'+o.entry+'</b></td><td>'+o.stop+' / '+o.tp1+'</td><td class="hide-m">'+(o.action==='ROTATE'?'<span class="pill am">ROTATE</span>':'<span class="pill pos">BUY</span>')+'</td></tr>';
      });h+='</tbody></table></div>';
    }
    if(d.signals&&d.signals.length>0){
      h+='<div class="section-card"><details><summary class="sc-summary"><span class="sc-sum-title">Today\\\'s Signals <span class="count">'+d.signals.length+' setup'+(d.signals.length===1?'':'s')+'</span></span></summary>'
        +'<table class="t" style="margin-top:.6rem"><thead><tr><th>Ticker</th><th>Score</th><th>Entry</th><th>Stop</th><th>TP1</th></tr></thead><tbody>';
      d.signals.forEach(function(s){var bg=s.score>=90?'#059669':s.score>=85?'#2563eb':'#f59e0b';
        h+='<tr><td><b>'+s.ticker+'</b></td><td><span class="pill-score" style="background:'+bg+'">'+s.score+'</span></td><td>'+s.entry+'</td><td class="neg">'+s.stop+'</td><td class="pos">'+s.tp1+'</td></tr>';
    ...
    if(d.positions&&d.positions.length>0){
      ...
      d.positions.forEach(function(p){var pnl=p.pnlPct!==undefined?p.pnlPct:(p.return_pct||0);var entry=p.entry||0;var stop=p.stop||0;
        if(stop>0&&entry>0)worstPct+=(stop-entry)/entry*100*allocPct;
        var tp=p.tp2||p.tp1||p.current_price||entry;if(entry>0&&tp>0)bestPct+=(tp-entry)/entry*100*allocPct;
        nowPct+=pnl*allocPct;
      });
      var r=bestPct-worstPct;var cp=r>0?Math.max(0,Math.min(100,(nowPct-worstPct)/r*100)):50;
      h+='<div class="section-card"><div class="sc-head"><h3>Open Positions <span class="count">'+d.positions.length+'/'+(mCfg.portfolioSize||'?')+'</span></h3><span class="sc-meta">avg P&L: <b class="'+(avgPnl>=0?'pos':'neg')+'">'+(avgPnl>0?'+':'')+avgPnl.toFixed(2)+'%</b></span></div>'
        +'<div class="scenario-bar-wrap"><div class="scenario-labels">'
      ...
      d.positions.forEach(function(p){var pnl=p.pnlPct!==undefined?p.pnlPct:(p.return_pct||0);var nowPrice=p.current_price||p.now_price||0;
        h+='<tr><td><b>'+p.ticker+'</b></td><td class="m hide-m">'+(p.scan_date?p.scan_date.slice(5):'\\u2014')+'</td><td class="hide-m">$'+(p.entry||0).toFixed(2)+'</td><td class="hide-m">'+(nowPrice>0?'$'+nowPrice.toFixed(2):'\\u2014')+'</td><td class="'+(pnl>=0?'pos':'neg')+'"><b>'+(pnl>0?'+':'')+pnl.toFixed(2)+'%</b></td><td class="neg hide-m">'+(p.stop&&p.stop!==0?'$'+p.stop.toFixed(2):'N/A')+'</td><td class="pos hide-m">'+(p.tp2&&p.tp2!==0?'$'+p.tp2.toFixed(2):'\\u2014')+'</td><td class="m">'+(p.days_remaining||0)+'d</td></tr>';
      });h+='</tbody></table></div>';
    }else{h+='<div class="section-card"><div class="sc-head"><h3>Open Positions</h3></div><p class="empty">No active positions</p></div>';}
    if(d.closedTrades&&d.closedTrades.length>0){
      h+='<div class="section-card"><details><summary class="sc-summary"><span class="sc-sum-title">Trade History <span class="count">'+d.closedTrades.length+' closed</span></span></summary>'
        +'<table class="t" style="margin-top:.6rem"><thead><tr><th>Ticker</th><th class="hide-m">End</th><th>P&L</th><th>Result</th></tr></thead><tbody>';
      d.closedTrades.slice().sort(function(a,b){return(b.scanDate||'').localeCompare(a.scanDate||'')}).forEach(function(t){var pnl=t.pnlPct||0;
        h+='<tr><td><b>'+t.ticker+'</b></td><td class="m hide-m">'+(t.scanDate?t.scanDate.slice(5):'\\u2014')+'</td><td class="'+(pnl>=0?'pos':'neg')+'"><b>'+(pnl>0?'+':'')+pnl.toFixed(2)+'%</b></td><td><span class="pill '+(pnl>=0?'pos':'neg')+'">'+(t.status||'CLOSED').toUpperCase()+'</span></td></tr>';
      });h+='</tbody></table></details></div>';
    }
    if((!d.signals||!d.signals.length)&&(!d.positions||!d.positions.length)&&(!d.closedTrades||!d.closedTrades.length)&&(!d.closeNow||!d.closeNow.length)&&(!d.orders||!d.orders.length)){
      h+='<div class="section-card"><div class="sc-head"><h3>No Activity</h3></div><p class="empty">No data for this date.</p></div>';
    }
    return h;
  }
  function tmRenderInto(tmr,d,mCfg,modeId){
    tmr.innerHTML=tmBuildHTML(d,mCfg,modeId);
    var color=mCfg.color||'#94a3b8';
