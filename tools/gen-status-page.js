...
document.addEventListener('DOMContentLoaded',function(){
  ...
  function tmRender(snap){
    var id=activeMode;
    var d=snap.modes[id];
    console.log('[TM] tmRender called, activeMode='+id+', hasData='+(!!d));
    if(!d){console.warn('[TM] No data for mode '+id+' in snapshot. Available:', Object.keys(snap.modes||{}));return;}
    console.log('[TM] stats:', JSON.stringify(d.stats));
    console.log('[TM] equity pts:', d.equity?d.equity.d.length:0);
    var mCfg = tmModesCfg.modes ? tmModesCfg.modes[id] : {};
    (function(){
      var panel=document.getElementById('p-'+id);
      if(!panel){console.error('[TM] panel p-'+id+' not found');return;}
      panel.style.transition='opacity .2s ease-in-out';
      panel.style.opacity='0.4';
      setTimeout(function(){
        var cfg=d.config||{};
        var stats=panel.querySelectorAll('.ps-v');
        console.log('[TM] .ps-v count:', stats.length);
        if(stats.length>=6){
        ...
        console.log('[TM] chartEl=', !!chartEl, 'id='+chartId);
        if(chartEl){
          var c=echarts.getInstanceByDom(chartEl);
          console.log('[TM] echarts instance=', !!c, 'equity=', !!(d.equity&&d.equity.d&&d.equity.d.length>0));
          if(d.equity&&d.equity.d&&d.equity.d.length>0){
            chartEl.parentElement.style.display='';
            var minV=Math.min.apply(null,d.equity.v);
            if(c){c.setOption({xAxis:{data:d.equity.d},yAxis:{min:Math.floor(minV)-1},series:[{data:d.equity.v}]});console.log('[TM] chart updated OK');}
            else{console.warn('[TM] NO echarts instance, chart not updated');}
          }else{ chartEl.parentElement.style.display='none'; console.log('[TM] no equity data, chart hidden'); }
        }
  ...
  tmInit();
  // Debug: auto-load TM snapshot by index (?tm=open&idx=N)
  (function(){
    var ps=new URLSearchParams(location.search);
    if(ps.get('tm')==='open'){
      setTimeout(function(){
        if(window.tmToggle)window.tmToggle();
        var idx=parseInt(ps.get('idx'));
        if(!isNaN(idx)&&idx>=0&&idx<tmDates.length){
          tmCurrentIdx=idx;
