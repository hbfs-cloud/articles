...
// Le balayage de regime.json est tronqué et classé par volume : un contrat peut disparaître d'un
// run à l'autre. Un marché OPTIONNEL absent doit faire disparaître son bloc, pas casser le build.
const pmIndex = (q, optional = false) => {
  const n = pmItems.findIndex(item => item.question === q);
  if (n < 0 && !optional) throw new Error(`marché absent : ${q}`);
  return n;
};
const pmHas = q => pmIndex(q, true) >= 0;
...
// Résolution paresseuse : offIndex est défini plus bas dans le fichier.
const pmCease = () => claim('pm_cease', OFF, `${OFFP}/${offIndex('Israel x Iran ceasefire')}/price`, PROB);
...
${dashboard.map(([label, close, move, note]) => `      <article class="dash-card"><div class="dash-label">${label}</div><div class="dash-value">${close}</div><div class="dash-move ${move.includes('−') ? 'down' : 'up'}">${move}</div><div class="dash-note">${note}</div></article>`).join('\n')}
  ...
  <section id="fed" class="section-shell" data-status="partial">
    ...
    <p class="section-intro">Sur les marchés de probabilité, la réunion du ${dFomc()} se répartit entre statu quo ${pmHold()} et hausse d&rsquo;un quart de point ${pmHike25()}. Les baisses ne sont plus cotées de façon exploitable. Les carnets donnent ${pmHoldBid()} contre ${pmHoldAsk()} sur le statu quo et ${pmHikeBid()} contre ${pmHikeAsk()} sur la hausse : le statu quo vient tout juste de passer devant, de deux à trois points, après avoir longtemps été derrière. L&rsquo;écart est réel mais mince.</p>
    <div class="two-col">
  ...
  <section id="watch" class="section-shell" data-status="validated">
    ...
    <ul class="checklist">
      ...
      <li><i class="fas fa-percent"></i><div><strong>La cote d&rsquo;une hausse de taux</strong><br>À ${pmHike25()}, contre ${pmHike7d()} sur sept jours. Un retour au-dessus du statu quo, actuellement ${pmHold()}, est le signal le plus clair que le marché change d&rsquo;avis.</div></li>
